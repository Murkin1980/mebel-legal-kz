...
Миграции 026–029 (объединённый файл `026_029_combined_stage6.sql`) применены к Supabase проекту `uctedpswcbcwufzegvhl` через SQL Editor. "Success. No rows returned."

> **Важно:** SQL Editor запускается с привилегированным доступом (service role) и **обходит RLS**. Подтверждение корректности RLS-политик даётся real-DB тестами (`legal-sources-realdb.test.ts`), которые выполняют запросы под anon-контекстом (без аутентификации) и проверяют, что RLS блокирует несанкционированный доступ. Все 42 real-DB теста пройдены.

**Real-DB валидация — 42 теста, все пройдены (включая 15 Stage 6):**

```
✓ contract_execution_phases table exists
✓ execution_checkpoints table exists
✓ execution_payments_summary table exists
✓ can SELECT/INSERT/UPDATE contract_execution_phases (drafting → internal_review → closed)
✓ can SELECT/INSERT/UPDATE execution_checkpoints (pending → in_progress → completed)
✓ can SELECT/INSERT/UPDATE execution_payments_summary (pending → partial → paid)
```
