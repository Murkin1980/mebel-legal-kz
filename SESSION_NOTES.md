...
**Статус:** Реализация завершена локально; миграция не применялась к staging/production

### Что сделано

- Инвентаризирован локальный `Murkin1980/mebeldocs-ai`: подтверждены Order, invoice, Money/date-utils и UI заказа; `node:fs`, AI и ESF не переносились.
- Принят ADR 002: заказ — верхний агрегат, договор необязателен, MebelLegal — внутренний legal-модуль.
- Добавлена одна additive migration с `orders`, `order_documents`, `order_deadlines`, `order_reminders`, RLS, явными grants и индексами.
- Добавлены `OrderSnapshotV1`, bigint-string money contract, рабочие дни и вычисляемые напоминания.
- Добавлены доменные команды создания заказа, документа и срока с tenant/role checks, audit и idempotency key.
- Добавлены список, создание и карточка заказа; legacy legal-модули сохранены.
- Запись заказа/документа/срока с напоминаниями и audit event объединена
  атомарными PostgreSQL RPC с фиксированным `search_path` и явными EXECUTE grants.
- Для счёта и акта добавлен скачиваемый печатный HTML-черновик; пользовательские
  значения экранируются, деньги форматируются из bigint-строки без float.
- Браузерный smoke-QA подтвердил страницу входа и перенаправление
  неавторизованного пользователя; брендинг входа приведён к MebelDocs.
- Staging, production, реальные данные и secrets не затронуты.

### Известные ограничения

- Миграция не применена к удалённой Supabase; runtime UI не проверен на реальной БД.
- Юридически готовый DOCX/PDF renderer и приватное файловое хранилище не входят
  в этот прототип; HTML-экспорт явно маркируется как черновик.
- Атомарные RPC не применялись и не проверялись на удалённой БД; real-DB
  проверка обязательна перед пилотом.
- Напоминания внутренние и вычисляемые; внешней доставки нет.
...
| `npm run test` | ✅ 233/233 |
| `npm run test:integration` | ✅ 119/119 |
| `npm run test:security` | ✅ 153/153 |
| `npm run build` | ✅ 32 route, включая экспорт счёта/акта |
| `npm run test:security:realdb` | Не запускался: запрещены внешние изменения/работа с удалёнными реальными данными |

### Foundation Check

- [x] Границы объединённого продукта зафиксированы ADR и FOUNDATION 1.1.
- [x] Tenant isolation и RLS сохранены в additive migration.
- [x] Серверная авторизация и role checks присутствуют.
- [x] Деньги передаются целыми тийинами строкой.
- [x] Legacy state machines не изменены.
- [x] Команды принимают idempotency key и создают audit events.
- [x] Audit log остаётся append-only.
- [x] Существующие подтверждённые данные не изменяются и не удаляются.
- [x] AI/LLM, ЭСФ, банк, претензии и публичные ссылки не добавлены.
- [x] В Git и тестах только синтетические данные, секретов нет.
- [x] Добавлены unit/integration/security tests по риску прототипа.
...
## Сессия: Надёжный staging pipeline

**Дата:** 23 июля 2026 года

### Что сделано

- CI переведён с Node.js 20 на Node.js 22.
- Автономные security-тесты отделены от real-DB security-тестов.
- Для push в `main` добавлен отдельный real-DB job, который использует GitHub
  Secrets при их наличии и безопасно пропускается без них.
- Добавлена единая локальная команда `npm run preflight`.
- Проверены Cloudflare OAuth, R2 bucket и наличие encrypted secret
  `SUPABASE_SERVICE_ROLE_KEY` без вывода его значения.
- Git и история коммитов проверены на типичные форматы service-role JWT,
  private key и OpenAI API key; совпадений не найдено.
- Wrangler dry-run успешно проверил assets, Worker bundle и bindings.
- Диагностика HTTP 500 сопоставлена с последующим исправлением в `5879f57`:
  OpenNext 1.20.1 не загружал Turbopack SSR chunks, поэтому production build
  переведён на `next build --webpack`.
...
## Сессия: Приёмка Roadmap 1–6 и закрытие детерминированных проверок/клиентского согласования

**Дата:** 9 августа 2026 года
**Статус:** локальная реализация завершена; внешняя приёмка миграций 031–033 ожидает доступности staging Supabase

### Что добавлено

- `031_rule_evaluations.sql` и `032_rule_evaluation_rpc.sql`: append-only запуски детерминированных правил для конкретной версии договорного пакета.
- Учитываются только утверждённые правила, связанные с утверждённой редакцией источника; проверяются обязательные поля, применимость и известные риски.
- Persistence RPC закрыта для браузерных ролей, работает через server-only service client, атомарно сохраняет результаты, summary и audit event.
- `033_client_approval_links.sql`: отзываемая ссылка, отдельный одноразовый challenge, максимум 5 попыток, одно решение на ссылку, идемпотентность по `command_id`.
- Token/challenge детерминированы HMAC для безопасного retry, в БД хранятся только hash; `APPROVAL_LINK_SECRET` остаётся server-only.
- Создание, отзыв и решение выполняются атомарными service-only RPC с audit; прямой SELECT/INSERT/UPDATE/DELETE клиентским ролям запрещён.
- Публичный `/approve/[token]` раскрывает только код и версию пакета, не показывает содержимое договора или tenant/PII данные, запрещён для индексации и referrer leakage.
- `ROADMAP.md` синхронизирован с фактической реализацией этапов 1–6 без удаления параллельно выпущенного Stage 6 execution lifecycle.
...
- Unit: ✅ 237/237.
- Integration: ✅ 124/124.
- Security (автономные): ✅ 158/158.
- Production build (`next build --webpack`): ✅, включая `/approve/[token]` и параллельно добавленные execution/orders маршруты.
- E2E/real-DB: ⚠️ не запущены — DNS staging Supabase `uctedpswcbcwufzegvhl.supabase.co` возвращает `ENOTFOUND`.
- Независимое ревью Codex Router: **APPROVE**, блокирующих замечаний нет.

### Foundation Check

- [x] Границы MebelLegal KZ / Interactive KP / MebelDocs AI не нарушены.
- [x] Tenant isolation и RLS сохранены; прямые browser writes к новым юридически значимым таблицам закрыты.
- [x] Серверная авторизация присутствует; service-role и `APPROVAL_LINK_SECRET` не попадают в браузер.
- [x] Деньги не хранятся в float/JavaScript number.
- [x] State transitions выполняются доменными командами.
- [x] Юридически значимые действия идемпотентны.
- [x] Audit log дополняется атомарно и остаётся append-only.
- [x] Решения и результаты проверок не перезаписываются, а сохраняются отдельными запусками.
- [x] AI не используется для юридического решения.
- [x] В Git, логах и fixtures не добавлены реальные договоры, реквизиты или секреты.
- [x] Добавлены unit/integration/security tests по риску изменений.
- [x] Изменения соответствуют явно разрешённым владельцем этапам 1–6.

### Непроверенная внешняя область

- Миграции 031–033 не применялись к удалённой БД в этой сессии.
- Real-DB и браузерные E2E необходимо повторить после восстановления DNS/доступности staging Supabase.
- Production deployment и реальные данные не использовались.
