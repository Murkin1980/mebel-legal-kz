...
**Статус:** Локальная реализация завершена. Ожидает применения миграций 026–029 к Supabase проекту и прогона real-DB тестов.
...
### Ожидание применения миграций к Supabase

Проект Supabase: `uctedpswcbcwufzegvhl` (тот же, что для Этапов 2–5).

**Dashboard:** https://supabase.com/dashboard/project/uctedpswcbcwufzegvhl

**SQL Editor:** → New query → вставить содержимое `supabase/migrations/026_029_combined_stage6.sql` → Run.

**Ожидаемый результат:** "Success. No rows returned" (если скрипт выполнен корректно — DDL не возвращает строк).

**Как проверить после применения:**

1. **Real-DB тесты** — запустить `npm run test:security` (включает `tests/security/legal-sources-realdb.test.ts` с добавленными 15 тестами для Stage 6):
   - `contract_execution_phases` — SELECT, INSERT (status='active', current_phase='drafting'), UPDATE (current_phase → internal_review, status → closed)
   - `execution_checkpoints` — SELECT, INSERT (status='pending'), UPDATE (→ in_progress, → completed с completed_at/completed_by)
   - `execution_payments_summary` — SELECT, INSERT (total_amount=10000000, paid_amount=0), UPDATE (paid_amount → 5000000/partial, → 10000000/paid)
   - После прогона: 15 новых real-DB тестов должны пройти

2. **SQL Editor — ручная проверка:**
   ```sql
   -- Проверка enum types
   SELECT enum_range(NULL::execution_phase_name);
   SELECT enum_range(NULL::checkpoint_status);
   SELECT enum_range(NULL::payment_status);

   -- Проверка таблиц
   SELECT table_name FROM information_schema.tables
   WHERE table_schema = 'public'
     AND table_name IN ('contract_execution_phases', 'execution_checkpoints', 'execution_payments_summary');

   -- Проверка RLS
   SELECT tablename, policyname FROM pg_policies
   WHERE tablename IN ('contract_execution_phases', 'execution_checkpoints', 'execution_payments_summary');

   -- Проверка индексов
   SELECT indexname FROM pg_indexes WHERE tablename IN (
     'contract_execution_phases', 'execution_checkpoints', 'execution_payments_summary'
   );
   ```

3. **После успешного применения обновить SESSION_NOTES.md:**
   - Статус: `Завершён и валидирован. Миграции 026–029 применены, real-DB тесты пройдены.`
